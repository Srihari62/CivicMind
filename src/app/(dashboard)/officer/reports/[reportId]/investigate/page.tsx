...
const MapViewer = dynamic(() => import("@/components/maps/MapViewer"), {
 ssr: false,
 loading: () => (
 <div className="h-[280px] w-full bg-slate-100 flex items-center justify-center border border-slate-200 rounded-2xl">
...
export default function OfficerInvestigationPage() {
 ...
 if (data.resolution.repairEvidence?.after && data.resolution.repairEvidence.after.length > 0) {
 setVerificationAfterPhotos(data.resolution.repairEvidence.after);
 } else if (data.resolution.afterMedia) {
 ...
 setVerificationAfterPhotos(assets);
 ...
 await saveResolutionDraftAction(
 profile.uid,
 report.id,
 profile.uid,
 draft,
 ...
 <p className="text-sm text-slate-500 max-w-xs">The incident report you are trying to access does not exist.</p>
 <Link href="/officer">
 ...
 <div className="min-h-screen bg-slate-50 text-zinc-100 p-4 sm:p-6 lg:p-8">
 ...
 <Link href="/officer">
 <Button variant="outline" className="rounded-full bg-slate-100 border-slate-200 text-slate-500 hover:text-slate-800 w-10 h-10 p-0 flex items-center justify-center">
 ...
 <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-800 mt-1">
 ...
 <span className="text-xs text-slate-500">Citizen Dashboard Live Sync Active</span>
 </div>
 </div>

 {/* Stepper */}
 <div className="w-full bg-slate-100/60 border border-slate-200 rounded-2xl p-4 sm:p-6 backdrop-blur-md">
 ...
 isActive ?"bg-blue-500 text-slate-800 shadow-[0_0_12px_rgba(59,130,246,0.5)]" :
"bg-slate-200 text-slate-500"
 ...
 <span className="text-[10px] text-slate-500 pl-8 hidden md:inline">{step.desc}</span>
 </div>
);
 })}
 </div>
 ...
 {(activeStep === 0) && (
 <div className="bg-slate-100 border border-slate-200 rounded-3xl p-6 sm:p-8 space-y-6">
 <div className="flex items-center gap-3 border-b border-slate-200 pb-4">
 <Shield className="w-6 h-6 text-blue-400" />
 <h2 className="text-lg font-bold text-slate-800">Assigned Case Review</h2>
 </div>

 <div className="space-y-4">
 <div>
 <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Citizen Description</span>
 <p className="mt-1 text-sm text-slate-600 leading-relaxed bg-slate-50 p-4 rounded-xl border border-slate-200">
 ...
 <span className="text-[10px] text-slate-500 uppercase font-bold block">Assigned Dept</span>
 <span className="text-sm font-semibold text-slate-600">{activeReport.ai?.assignment?.department ||"General"}</span>
 </div>
 <div className="p-3 bg-slate-50 rounded-xl border border-slate-200">
 <span className="text-[10px] text-slate-500 uppercase font-bold block">Duplicate Score</span>
 ...
 <Button onClick={handleAccept} className="flex-1 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-slate-800 font-bold h-12 rounded-xl">
 Accept Case & Start Workflow
 </Button>
 <Button onClick={() => setIsRejectModalOpen(true)} variant="outline" className="border-slate-200 text-slate-500 hover:text-slate-800 h-12 px-6 rounded-xl">
 Decline & Reassign
 </Button>
 </div>
 </div>
 ...
 <h2 className="text-lg font-bold text-slate-800">En Route to Location</h2>
 </div>

 <div className="p-6 bg-slate-50 rounded-2xl border border-slate-200 space-y-4 relative overflow-hidden">
 <div className="flex justify-between items-start">
 <div>
 <span className="text-2xl font-bold text-slate-800">1.8 km</span>
 <span className="text-xs text-slate-500 block">Estimated Distance</span>
 </div>
 <div className="text-right">
 <span className="text-2xl font-bold text-blue-400">6 mins</span>
 <span className="text-xs text-slate-500 block">Est. Driving Time</span>
 </div>
 </div>

 <div className="border border-slate-200 rounded-xl p-4 bg-slate-100/60 text-xs space-y-2">
 <div className="flex items-center gap-2 text-slate-600">
 <MapPin className="w-4 h-4 text-blue-400" />
 <span>Navigate: <strong>Head north on Main St towards Incident Site</strong></span>
 </div>
 <p className="text-[10px] text-slate-500">Auto-refreshing GPS coordinate alignment is active.</p>
 </div>
 ...
 <Button onClick={handleStartTravel} className="flex-1 bg-blue-600 hover:bg-blue-700 text-slate-800 font-bold h-12 rounded-xl">
 ...
 <h2 className="text-lg font-bold text-slate-800">Field Investigation Notes & Severity</h2>
 </div>
 <span className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Investigation Active</span>
 </div>

 <div className="space-y-4">
 <div>
 <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Field Findings &amp; Inspection Notes</label>
 <VoiceInput
 multiline
 rows={4}
 ...
 <div>
 <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Observed Severity</label>
 <select
 value={observedSeverity}
 onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setObservedSeverity(e.target.value)}
 className="flex h-10 w-full items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-zinc-100 placeholder:text-slate-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
 >
 ...
 <div>
 <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Safety Risk Level</label>
 <select
 value={safetyRisk}
 ...
 <div>
 <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Materials Needed / Scheduled</label>
 <Input
 ...
 className="bg-slate-50 border-slate-200 rounded-xl text-slate-700"
 />
 </div>

 {/* Investigation Photos Upload */}
 <div className="space-y-2">
 <label className="text-xs font-bold text-slate-500 uppercase block">Investigation Evidence Photos</label>
 <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
 {investigationPhotos.map((photo, idx) => (
 <div key={photo.id || idx} className="aspect-square rounded-xl overflow-hidden border border-slate-200 relative group">
 {/* eslint-disable-next-line @next/next/no-img-element */}
 <img src={photo.url} alt="Investigation" className="w-full h-full object-cover" />
 </div>
))}
 <label className="aspect-square rounded-xl border border-dashed border-slate-200 clay-card p-5 hover:bg-slate-50 hover:border-slate-200 transition flex flex-col items-center justify-center gap-2 cursor-pointer">
 ...
 <div className="flex gap-4 pt-4 border-t border-slate-200">
 <Button onClick={handleSaveInvestigation} disabled={savingInvestigation} className="bg-slate-200 hover:bg-zinc-700 text-slate-700 h-12 px-6 rounded-xl">
 {savingInvestigation ?"Saving..." :"Save Findings"}
 </Button>
 <Button onClick={handleStartRepair} disabled={startingRepair} className="flex-1 bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-slate-800 font-bold h-12 rounded-xl">
 ...
 <h2 className="text-lg font-bold text-slate-800">Repair Operations</h2>
 </div>
 ...
 <div className="space-y-4">
 <div>
 <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Work Performed Description</label>
 <VoiceInput
 multiline
 rows={3}
 ...
 <div>
 <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Materials Used (Finalized)</label>
 <Input
 ...
 <div className="grid grid-cols-2 gap-4">
 <div>
 <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Labour/Crew Size</label>
 <Input
 type="number"
 value={labourCount}
 ...
 <div>
 <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Repair Cost ($)</label>
 <Input
 type="number"
 value={repairCost}
 ...
 <div className="space-y-2">
 <label className="text-xs font-bold text-slate-500 uppercase block">Repair Media (Include AFTER evidence)</label>
 <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
 ...
 <span className="text-xs font-bold text-slate-500 uppercase block border-b border-slate-200 pb-2">
 ...
 className="rounded border-slate-200 bg-slate-100 text-amber-500 focus:ring-amber-500 focus:ring-offset-zinc-950"
 />
 ...
 <div className="flex gap-4 pt-4 border-t border-slate-200">
 <Button onClick={handleSaveRepairProgress} disabled={savingRepairProgress} className="bg-slate-200 hover:bg-zinc-700 text-slate-700 h-12 px-6 rounded-xl">
 ...
 <h2 className="text-lg font-bold text-slate-800">AI Verification & Final Resolution Review</h2>
 </div>
 ...
 <span className="text-[10px] text-slate-500 uppercase block font-bold">Before Media</span>
 {activeReport.evidence?.media && activeReport.evidence.media.length > 0 ? (
 // eslint-disable-next-line @next/next/no-img-element
 <img src={activeReport.evidence.media[0].url} alt="Before" className="aspect-video w-full object-cover rounded-lg border border-slate-200" />
) : (
 <div className="aspect-video w-full bg-slate-100 flex items-center justify-center text-xs text-zinc-600 rounded-lg">No photos</div>
)}
 </div>
 <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
 <span className="text-[10px] text-slate-500 uppercase block font-bold">After Media (Required)</span>
 {verificationAfterPhotos.length > 0 ? (
 // eslint-disable-next-line @next/next/no-img-element
 <img src={verificationAfterPhotos[0].url} alt="After" className="aspect-video w-full object-cover rounded-lg border border-slate-200" />
) : (
 <div className="aspect-video w-full bg-slate-100 flex items-center justify-center text-xs text-zinc-600 rounded-lg">No photos</div>
)}
 <label className="mt-2 block w-full text-center py-2 border border-dashed border-slate-200 hover:border-slate-200 bg-slate-100/40 rounded-lg text-xs cursor-pointer text-slate-500">
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
 <div>
 <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Resolution Review Notes</label>
 <VoiceInput
 multiline
 rows={4}
 ...
 <div>
 <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Final Materials Used</label>
 <VoiceInput
 multiline
 rows={4}
 ...
 <span className="text-xs font-bold text-slate-600 uppercase">Gemini 3.1 Flash Lite Verification</span>
 </div>
 <Button
 onClick={handleAiVerification}
 disabled={aiRunning || verificationAfterPhotos.length === 0}
 className="bg-cyan-600 hover:bg-cyan-700 text-slate-800 font-semibold h-10 px-4 rounded-xl flex items-center gap-2 text-xs"
 >
 {aiRunning ? (
 <>
...
"Trigger AI Audit"
 ...
 <span className="text-slate-500 block">AI Verified Repair?</span>
 ...
 <span className="text-sm font-bold text-slate-800">{Math.round(aiResults.confidence * 100)}%</span>
 </div>
 </div>

 <div className="space-y-3">
 <div>
 <label className="text-slate-500 font-bold block mb-1">AI Technical Summary (Editable)</label>
 <textarea
 ...
 className="flex min-h-[60px] w-full rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-sm text-zinc-100 placeholder:text-slate-500 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-blue-500"
 />
 </div>
 <div>
 <label className="text-slate-500 font-bold block mb-1">AI Citizen Explanation (Editable)</label>
 <textarea
 ...
 <div>
 <label className="text-slate-500 font-bold block mb-1">AI Admin Audit Summary (Editable)</label>
 <textarea
 ...
 <div className="flex gap-4 pt-4 border-t border-slate-200">
 <Button onClick={handleSaveResolutionDraft} disabled={savingDraft} className="bg-slate-200 hover:bg-zinc-700 text-slate-700 h-12 px-6 rounded-xl">
 ...
 <h2 className="text-lg font-bold text-slate-800">Case Resolved & Finalized</h2>
 </div>
 ...
 <h3 className="font-bold text-slate-800 text-lg">Case Successfully Closed</h3>
 <p className="text-xs text-slate-500 max-w-sm">Officer workload has been updated, and point rewards have been distributed.</p>
 </div>
 <div className="flex items-center gap-3 bg-emerald-500/20 border border-emerald-500/30 px-4 py-2 rounded-xl">
 <Award className="w-5 h-5 text-emerald-400" />
 <div>
 <span className="text-xs text-slate-500 block uppercase font-bold tracking-wider">Rewards</span>
 <span className="text-sm font-bold text-slate-800">+10 Performance Score</span>
 </div>
 </div>
 </div>
 ...
 <span className="text-xs font-bold text-slate-500 uppercase tracking-widest block">Incident Location Map</span>
 ...
 <div className="h-[280px] bg-slate-50 border border-slate-200 rounded-2xl flex items-center justify-center text-xs text-zinc-600">No GPS Details</div>
)}

 <div className="space-y-2 text-xs">
 <span className="text-slate-500 uppercase block font-bold">Address</span>
 ...
 {/* Timeline feed */}
 <div className="bg-slate-100 border border-slate-200 rounded-3xl p-6 space-y-4">
 <span className="text-xs font-bold text-slate-500 uppercase tracking-widest block">Incident Timeline</span>
 <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2">
 {activeReport.timeline && activeReport.timeline.length > 0 ? (
 activeReport.timeline.map((event, idx) => (
 <div key={idx} className="flex gap-3 relative border-l border-slate-200 pl-4 pb-4 last:pb-0">
 ...
 <span className="text-xs font-bold text-slate-600">{event.action}</span>
 {event.note && <p className="text-[10px] text-slate-500 mt-0.5">{event.note}</p>}
 </div>
 </div>
))
) : (
 <span className="text-xs text-slate-500 italic block">No timeline events found.</span>
)}
 </div>
 </div>
 ...
 <div className="bg-slate-100 border border-slate-200 rounded-3xl p-6 w-full max-w-md space-y-6 shadow-2xl">
 <div>
 <h3 className="text-lg font-bold text-slate-800">Decline & Reassign Case</h3>
 <p className="text-xs text-slate-500 mt-1">Please select the reason for reassigning this report to another officer or department.</p>
 </div>

 <div className="space-y-3">
 <label className="text-xs font-bold text-slate-500 uppercase">Reason for Reassignment</label>
 <select
 value={rejectReason}
 ...
 <Button onClick={handleReject} disabled={rejecting || !rejectReason} className="flex-1 bg-rose-600 hover:bg-rose-700 text-slate-800 font-bold h-11 rounded-xl">
 {rejecting ?"Declining..." :"Decline Assignment"}
 </Button>
 <Button onClick={() => setIsRejectModalOpen(false)} variant="outline" className="border-slate-200 text-slate-500 hover:text-slate-800 h-11 rounded-xl">
 Cancel
 </Button>
 </div>
 </div>

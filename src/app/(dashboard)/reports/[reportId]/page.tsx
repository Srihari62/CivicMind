...
export default function ReportDetailsPage() {
 ...
 <div className="flex min-h-screen flex-col bg-slate-50 text-slate-800 font-sans selection:bg-blue-600/30 selection:text-slate-800">
 {/* Navigation Bar */}
 <header className="border-b border-slate-200 bg-white/80 backdrop-blur-2xl shadow-sm sticky top-0 z-30">
 ...
 <Link href="/community" className="text-xs text-slate-500 hover:text-slate-800 font-semibold transition-colors flex items-center gap-1.5">
 <ArrowLeft className="w-3.5 h-3.5" /> Community Feed
 </Link>
 <span className="text-zinc-700">|</span>
 <Link href="/dashboard" className="text-xs text-slate-500 hover:text-slate-800 font-semibold transition-colors flex items-center gap-1.5">
 Back to Dashboard
 </Link>
 </div>
 </div>
 ...
 <span className="font-bold text-slate-800">Report submitted successfully.</span>
 ...
 <span className="text-sm font-semibold text-slate-500">Streaming live report data...</span>
 </div>
) : error || !report ? (
 ...
 <span className="text-xs uppercase tracking-wider font-semibold px-2.5 py-0.5 rounded bg-slate-200 text-slate-600 border border-slate-200">
 ...
 <div className="flex flex-col gap-0.5">
 <span className="font-bold text-slate-800">Linked Repost</span>
 ...
 <p className="text-sm text-slate-700 whitespace-pre-wrap leading-relaxed border border-slate-200 p-5 rounded-2xl clay-card p-5 backdrop-blur-md">
 {report.metadata.description}
 </p>
 </div>

 {/* Meta details grid */}
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border border-slate-200 p-5 rounded-2xl clay-card p-5 backdrop-blur-md text-sm">
 <div className="flex flex-col gap-1">
 <span className="text-xs text-slate-500 uppercase tracking-wider select-none font-semibold">Address / Location</span>
 <span className="font-semibold text-slate-700">{report.location.formattedAddress}</span>
 </div>
 <div className="flex flex-col gap-1">
 <span className="text-xs text-slate-500 uppercase tracking-wider select-none font-semibold">Coordinates</span>
 <span className="font-medium text-xs font-mono text-slate-600">
 Lat: {report.location.latitude.toFixed(6)}, Lng: {report.location.longitude.toFixed(6)}
 </span>
 </div>
 {report.location.city && (
 <div className="flex flex-col gap-1">
 <span className="text-xs text-slate-500 uppercase tracking-wider select-none font-semibold">Region / City / State</span>
 ...
 <div className="flex flex-col gap-1">
 <span className="text-xs text-slate-500 uppercase tracking-wider select-none font-semibold">Initial Urgency Priority</span>
 <span className="font-bold text-xs bg-slate-200 text-slate-600 px-2 py-0.5 rounded border border-slate-200 inline-block w-fit capitalize">
 ...
 <div className="absolute top-2 left-2 bg-black/70 border border-slate-200 text-[8px] text-slate-800 px-2 py-0.5 rounded font-mono select-none">
 ...
 <Link href="/community">
 <Button variant="outline" size="sm" className="border-slate-200 hover:bg-slate-100 text-slate-600">
 Go to Community Feed
 </Button>
 </Link>
 <Link href="/dashboard">
 <Button size="sm" className="bg-blue-600 hover:bg-blue-500 text-slate-800 font-bold">
 Return to Dashboard
 </Button>
 </Link>
 </div>

...
export default function ReportComments({ reportId }: ReportCommentsProps) {
      ...
      <div className="flex flex-col gap-3 relative">
        <div className="flex gap-3 items-start">
          ...
          <div className="w-9 h-9 rounded-full bg-zinc-800 border border-white/10 overflow-hidden shrink-0 flex items-center justify-center">
            {comment.userAvatarUrl ? (
              ...
              <span className="text-xs font-bold text-zinc-400">{comment.userName[0]?.toUpperCase()}</span>
            )}
          </div>

          {/* Comment Bubble */}
          <div className="flex-1 bg-zinc-900/40 border border-white/5 p-4 rounded-2xl relative shadow-md">
            {/* Header info */}
            <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {comment.edited && (
                  <span className="text-[10px] text-zinc-500 italic font-mono">(edited)</span>
                )}
              </div>
              ...
              <div className="flex items-center gap-2">
                {user && (
                  <button
                    ...
                    className="text-zinc-400 hover:text-blue-400 transition p-1 rounded hover:bg-white/5"
                    title="Reply"
                  >
                ...
                {isOwner && !isEditing && (
                  <>
                    <button
                      ...
                      className="text-zinc-400 hover:text-amber-400 transition p-1 rounded hover:bg-white/5"
                      title="Edit"
                    >
                    ...
                    <button
                      ...
                      className="text-zinc-400 hover:text-rose-400 transition p-1 rounded hover:bg-white/5"
                      title="Delete"
                    >
            ...
            {isEditing ? (
              ...
              <div className="space-y-2">
                <p className="text-sm text-zinc-200 leading-relaxed whitespace-pre-wrap">{comment.content}</p>
                {comment.photoUrl && (
                  <div className="relative max-w-xs rounded-xl overflow-hidden border border-white/10 bg-zinc-950/60 aspect-video mt-2 group">
                    <img
        ...
        <AnimatePresence>
          {isReplying && (
            ...
            >
              <div className="w-7 h-7 rounded-full bg-zinc-800 border border-white/10 shrink-0 flex items-center justify-center">
              ...
              <div className="flex-1 bg-zinc-950 border border-white/10 p-3.5 rounded-2xl flex flex-col gap-2.5">
                ...
                {/* Reply Photo Preview */}
                {replyPhoto && (
                  <div className="relative w-24 aspect-square border border-white/10 rounded-lg overflow-hidden bg-zinc-900 group shrink-0">
                    <img src={replyPhoto} alt="Attachment Preview" className="object-cover w-full h-full" />
                    <button
                      type="button"
                      onClick={() => setReplyPhoto(null)}
                      className="absolute top-1 right-1 bg-black/80 hover:bg-black text-white rounded-full p-0.5"
                    >
                      <X className="w-3 h-3" />
                    </button>
                  </div>
        ...
        {comment.replies.length > 0 && (
          <div className="pl-12 flex flex-col gap-4 border-l border-white/5 mt-2">
  ...
  return (
    <div className="w-full rounded-2xl border border-white/10 bg-black/40 p-6 backdrop-blur-md shadow-2xl flex flex-col gap-6">
      ...
      {/* Main Comment Form */}
      {user ? (
        <form onSubmit={handleSubmitRootComment} className="flex gap-3 items-start bg-zinc-900/30 border border-white/5 p-4 rounded-2xl shadow-inner">
          <div className="w-9 h-9 rounded-full bg-zinc-800 border border-white/10 overflow-hidden shrink-0 flex items-center justify-center">
            {profile?.avatarUrl ? (
              <img src={profile.avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
            ) : (
              <span className="text-xs font-bold text-zinc-400">{profile?.displayName?.[0]?.toUpperCase() || user.email?.[0]?.toUpperCase()}</span>
            )}
          </div>
            ...
            <div className="flex items-center justify-between border-t border-white/5 pt-3">
              <div>
                ...
                <button
                  ...
                  className="text-zinc-400 hover:text-white transition disabled:opacity-50 flex items-center gap-1.5 text-xs font-semibold"
                >
                  {uploadingPhoto ? (
                    <>
        ...
        <div className="text-center p-6 border border-dashed border-white/10 rounded-2xl text-xs text-zinc-500">
      ...
      {loading ? (
        <div className="flex flex-col items-center py-6 gap-2">
          ...
          <span className="text-xs text-zinc-500 font-medium">Streaming comments...</span>
        </div>
      ) : threaded.length === 0 ? (
        <div className="text-center py-8 text-xs text-zinc-500 border border-dashed border-white/5 rounded-2xl">

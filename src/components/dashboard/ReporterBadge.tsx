...
export default function ReporterBadge({ userId }: ReporterBadgeProps) {
 ...
 if (loading) {
 return <div className="w-24 h-4 bg-slate-200 animate-pulse rounded" />;
 }

 return (
 <div className="flex items-center gap-1.5 bg-white/5 border border-slate-200 px-2.5 py-0.5 rounded-full text-[10px] text-zinc-350 select-none">
 {avatar ? (
 <img src={avatar} alt="Reporter Avatar" className="w-3.5 h-3.5 rounded-full object-cover shrink-0" />
) : (
 <User className="w-3 h-3 text-slate-500 shrink-0" />
)}
 <span>Reported by: <strong className="text-slate-700 font-semibold">{name}</strong></span>
 </div>
);
}

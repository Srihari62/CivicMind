...
export default function AIAnalysisCard({ report }: AIAnalysisCardProps) {
 ...
 if (isProcessing) {
 return (
 <div className="w-full rounded-2xl clay-card p-6 relative overflow-hidden">
 {/* Shimmer background */}
 ...
 <div className="h-8 w-full bg-slate-200/50 rounded-lg animate-pulse" />
 </div>
 <div className="space-y-2">
 ...
 <div className="h-4 w-[90%] bg-slate-200/60 rounded animate-pulse" />
 <div className="h-4 w-[85%] bg-slate-200/50 rounded animate-pulse" />
 </div>
 </div>
 </div>
);
 ...
 <div>
 <h3 className="text-lg font-bold text-slate-800">AI Diagnostics Report</h3>
 <span className="text-xs text-slate-500 font-mono">Model: {verification.verificationModel ||"Gemini 2.5 Flash"} • v{verification.verificationVersion ||"1.0"}</span>
 </div>
 </div>
 ...
 <span className="text-slate-800 font-mono font-bold">{confidence}%</span>
 </div>
 ...
 <span className="text-slate-800 font-mono font-bold">{trustScore}/100</span>
 </div>
 <div className="h-3 w-full bg-slate-200 rounded-full overflow-hidden">
 <motion.div 
 ...
 <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-b border-slate-200 py-4">
 {/* Fake Media Probability */}
 <div className="flex items-center justify-between text-xs">
 <span className="text-slate-500 font-medium">Fake Media Probability:</span>
 <div className="flex items-center gap-2">
 <div className="w-24 h-1.5 bg-slate-200 rounded-full overflow-hidden">
 <div className="h-full bg-rose-500" style={{ width: `${fakeProb}%` }} />
 </div>
 <span className={`font-mono font-bold ${fakeProb > 30 ?"text-rose-400" :"text-slate-600"}`}>{fakeProb}%</span>
 </div>
 </div>

 {/* Duplicate Probability */}
 <div className="flex items-center justify-between text-xs">
 <span className="text-slate-500 font-medium">Duplicate Match Probability:</span>
 <div className="flex items-center gap-2">
 <div className="w-24 h-1.5 bg-slate-200 rounded-full overflow-hidden">
 <div className="h-full bg-amber-500" style={{ width: `${dupProb}%` }} />
 </div>
 <span className={`font-mono font-bold ${dupProb > 30 ?"text-amber-400" :"text-slate-600"}`}>{dupProb}%</span>
 </div>
 </div>
 </div>
 ...
 <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-500">AI Verification Summary</h4>
 <p className="text-sm text-slate-700 bg-white/5 border border-slate-200 p-4 rounded-xl leading-relaxed">
 ...
 <div className="space-y-2">
 <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-500">Detected Visual Objects</h4>
 <div className="flex flex-wrap gap-1.5">
 {assistant.detectedObjects.map((obj, idx) => (
 <span key={idx} className="px-2 py-0.5 text-[10px] font-semibold bg-slate-200 text-slate-600 rounded border border-slate-200 font-mono">
 {obj}
 </span>
))}
 </div>

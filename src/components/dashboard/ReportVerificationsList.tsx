...
export default function ReportVerificationsList({ reportId }: ReportVerificationsListProps) {
 ...
 if (loading) {
 return (
 <div className="flex flex-col items-center py-6 gap-2 border border-slate-200 rounded-2xl clay-card p-5">
 <Loader2 className="w-5 h-5 text-blue-500 animate-spin" />
 <span className="text-xs text-slate-500 font-medium">Loading community verifications...</span>
 </div>
);
 }
 ...
 return (
 <div className="text-center py-8 text-xs text-slate-500 border border-dashed border-slate-200 rounded-2xl bg-slate-50 border-slate-200">
 ...
 <div
 key={v.id}
 className="border border-slate-200 rounded-2xl p-5 bg-slate-100/40 backdrop-blur-md flex flex-col md:flex-row gap-5 items-start justify-between"
 >
 {/* Left Column: Details */}
 ...
 <p className="text-xs text-slate-600 leading-relaxed font-medium clay-card p-5 border border-slate-200 p-3 rounded-xl mt-1.5 flex gap-2 items-start">
 <MessageSquare className="w-3.5 h-3.5 text-slate-500 shrink-0 mt-0.5" />
 <span>{v.verificationComment}</span>
 </p>
) : (
 <p className="text-xs text-slate-500 italic mt-1 font-medium">No description provided.</p>
)}
 </div>
 </div>
 ...
 {v.verificationPhoto && (
 <div className="relative w-full md:w-32 aspect-video md:aspect-square border border-slate-200 rounded-xl overflow-hidden bg-slate-50/60 group shrink-0 self-center">
 <img
 ...
 />
 <div className="absolute top-1.5 left-1.5 bg-black/75 border border-slate-200 text-[8px] text-slate-800 px-2 py-0.5 rounded font-mono select-none">
 EVIDENCE PHOTO
 </div>
 </div>
)}

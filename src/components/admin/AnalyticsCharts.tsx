...
export default function AnalyticsCharts({ reports }: AnalyticsChartsProps) {
 ...
 <span className="capitalize text-slate-600">{c.label}</span>
 ...
 <span className="text-xs text-slate-500 italic text-center py-10">No department logs on record.</span>
) : (
 departments.map((d) => {
 ...
 <div className="clay-card p-6 flex flex-col md:flex-row items-center justify-between gap-6">
 ...
 <span className="text-2xl font-black text-slate-800 font-mono">{reports.length}</span>
 <span className="text-[9px] text-slate-500 uppercase font-bold tracking-widest">Reports</span>
 </div>
 </div>
 </div>
 ...
 <span className="font-bold text-slate-600 uppercase tracking-wider">{key}</span>
 ...
 {/* 5. Reports by Day (Last 30 Days) - SVG Area Sparkline */}
 <div className="clay-card p-6 flex flex-col gap-4 lg:col-span-2">
 ...
 <div className="relative w-full h-[180px] bg-slate-50 border-slate-200 rounded-xl border border-slate-200 overflow-hidden p-4">
 <svg
 ...
 cy={p.y}
 r="4"
 className="fill-blue-500 stroke-slate-100 stroke-2 hover:r-6 cursor-pointer transition-all"

    ...
    <div className="w-full rounded-2xl border border-white/10 bg-black/40 p-6 backdrop-blur-md shadow-2xl space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/5 pb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-lg font-bold text-white">Operational Trend Analytics</h3>
            <span className="text-xs text-zinc-400">Category volume metrics compared across time boundaries</span>
          </div>
        </div>

        {/* Tab-styled buttons */}
        <div className="flex bg-zinc-900 border border-white/10 rounded-xl p-0.5 self-start sm:self-auto">
          <button
            onClick={() => setPeriod("day")}
            className={`px-3 py-1 text-xs font-bold rounded-lg transition ${
              period === "day" ? "bg-yellow-500 text-black shadow-md" : "text-zinc-400 hover:text-white"
            }`}
          >
            Daily
          </button>
          <button
            ...
            className={`px-3 py-1 text-xs font-bold rounded-lg transition ${
              period === "week" ? "bg-yellow-500 text-black shadow-md" : "text-zinc-400 hover:text-white"
            }`}
          >
            Weekly
          </button>
          <button
            ...
            className={`px-3 py-1 text-xs font-bold rounded-lg transition ${
              period === "month" ? "bg-yellow-500 text-black shadow-md" : "text-zinc-400 hover:text-white"
            }`}
          >
            Monthly
          </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {targetCategories.map((cat) => {
          ...
          return (
            <div 
              key={cat.key} 
              className="p-4 rounded-xl border border-white/5 bg-white/[0.01] hover:border-white/10 transition-all flex flex-col justify-between gap-3 relative group overflow-hidden"
            >
              <div className="flex justify-between items-start gap-2">
                <span className="text-xs font-extrabold text-zinc-300 group-hover:text-white transition">
                  {cat.label}
                </span>
              ...
              <div className="flex items-baseline gap-1.5 mt-2">
                ...
                <div className="flex items-center">
                  ...
                  {!isUp && !isDown && <Minus className="w-4 h-4 text-zinc-500" />}
                </div>
              </div>

              <div className="border-t border-white/5 pt-2 flex items-center justify-between text-[9px] text-zinc-500 font-medium">
      ...
      <div className="p-3.5 rounded-xl bg-white/[0.02] border border-white/5 flex items-start gap-2 text-xs text-zinc-400">

...
export default function IncidentHeatmap({ reports }: { reports: CivicReport[] }) {
  ...
  if (!mounted) {
    return (
      <div className="h-[500px] rounded-2xl border border-white/10 bg-zinc-950/40 flex items-center justify-center animate-pulse">
        <span className="text-zinc-500 font-semibold text-sm">Initializing Command Map Engine...</span>
      </div>
    );
  }
  ...
  return (
    <div className="w-full rounded-2xl border border-white/10 bg-black/40 p-6 backdrop-blur-md shadow-2xl space-y-6 relative z-10">
      {/* Control Title Bar */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-white/5 pb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-lg font-bold text-white">Geospatial Incident Intelligence</h3>
            <span className="text-xs text-zinc-400">Real-time command center telemetry plotting</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <h4 className="text-xs font-semibold text-zinc-500 uppercase tracking-widest">Category Layers</h4>
          <div className="space-y-2 border border-white/5 p-4 rounded-xl bg-white/[0.02]">
            {Object.keys(categoryFilters).map((catKey) => (
              <label key={catKey} className="flex items-center gap-2.5 py-1 text-xs text-zinc-300 cursor-pointer hover:text-white transition">
                <input
                  type="checkbox"
                  checked={categoryFilters[catKey]}
                  onChange={(e) => setCategoryFilters({ ...categoryFilters, [catKey]: e.target.checked })}
                  className="rounded border-zinc-800 bg-zinc-900 text-red-500 focus:ring-red-500/40 w-4 h-4"
                />
          ...
          {/* Severity legend */}
          <div className="space-y-2 border border-white/5 p-4 rounded-xl bg-white/[0.02]">
            <h5 className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Severity Legend</h5>
...
function PopupReportDetail({ report, color }: { report: CivicReport; color: string }) {
  ...
  return (
    <div className="p-3 max-w-[240px] text-xs font-sans text-zinc-300 space-y-2.5">
      <div className="space-y-1">
        <div className="flex items-center gap-1.5">
          ...
          />
          <span className="font-black text-white text-xs block truncate max-w-[180px]">
      ...
      <div className="border-t border-zinc-800 pt-2 flex items-center justify-between gap-2">

...
export const MunicipalAssistant: React.FC<MunicipalAssistantProps> = ({ reports, users }) => {
  ...
  if (!reports || reports.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 border border-dashed border-white/10 rounded-2xl bg-zinc-950/20">
        ...
        <div className="flex-1">
          {activeTab === "dept_health" && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {Object.values(analytics.departments).map((dept) => (
                  <div
                    ...
                    className="border border-white/5 bg-zinc-950/40 rounded-2xl p-4.5 flex flex-col gap-3 transition-all duration-300 hover:border-red-500/20 hover:bg-zinc-900/20 cursor-pointer group"
                    title={`Click to copy: Explain ${dept.name} department health`}
                  >
                    {/* Header */}
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        <Building2 className="w-4 h-4 text-zinc-400 group-hover:text-red-400 transition-colors" />
                        <span className="font-extrabold text-xs text-white uppercase tracking-wider">
                          {dept.name}
                        </span>
                      </div>
                      <span className="text-[9px] font-bold text-zinc-500 group-hover:text-zinc-350 px-2 py-0.5 rounded bg-zinc-900 border border-white/5">
                        {dept.totalCount} Reports
                      </span>
                    </div>

                    {/* Scorecard KPIs */}
                    <div className="grid grid-cols-3 gap-2 text-center border-t border-b border-white/5 py-2 my-1 text-xs">
                      <div>
                        <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Pending</div>
                        <div className="font-bold font-mono text-zinc-200 mt-0.5">{dept.pendingReports}</div>
                      </div>
                      <div>
                        <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Backlog</div>
                        <div className="font-bold font-mono text-zinc-200 mt-0.5">{dept.backlogSize}</div>
                      </div>
                      <div>
                        <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Workload</div>
                        <div className="font-bold font-mono text-zinc-200 mt-0.5">{dept.workload}</div>
                      </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center text-[10px] text-zinc-450 font-semibold">
                      <span className="flex items-center gap-1">
                        ...
                        Avg SLA: <strong className="font-mono text-zinc-300">{dept.averageResolutionTime > 0 ? `${dept.averageResolutionTime}h` : "N/A"}</strong>
                      </span>
                      <span className="flex items-center gap-1">
                        <ShieldCheck className="w-3 h-3 text-emerald-400" />
                        Trust: <strong className="font-mono text-zinc-300">{dept.trustScoreAverage > 0 ? `${dept.trustScoreAverage}%` : "N/A"}</strong>
                      </span>
                    </div>
                  </div>
                ))}
              ...
              <div className="border border-white/5 bg-zinc-900/30 rounded-2xl p-4 flex gap-3 items-start backdrop-blur-sm">
          ...
          {activeTab === "hotspots_officers" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="border border-white/5 bg-zinc-950/40 rounded-2xl p-5 flex flex-col gap-4">
                ...
                {analytics.hotspots.length === 0 ? (
                  <p className="text-[10px] text-zinc-500 text-center py-6 font-bold">No hotspots identified yet.</p>
                ) : (
                  <div className="space-y-3 overflow-y-auto max-h-[400px] pr-1">
                    {analytics.hotspots.slice(0, 6).map((spot, idx) => (
                      <div key={idx} className="flex justify-between items-start gap-2 p-2.5 rounded-xl border border-white/5 bg-zinc-900/20 text-xs hover:border-red-500/10 transition-all">
                        <div className="flex flex-col gap-0.5">
                          <span className="font-extrabold text-zinc-200 line-clamp-1">{spot.locality}</span>
                          <span className="text-[9px] font-mono text-zinc-500">
                            Lat: {spot.latitude.toFixed(4)}, Lng: {spot.longitude.toFixed(4)}
                          </span>
                          <div className="flex flex-wrap gap-1 mt-1.5">
                            {Object.entries(spot.categories).map(([cat, cnt]) => (
                              <span key={cat} className="text-[8px] font-bold text-zinc-400 bg-zinc-950 border border-white/5 px-1.5 py-0.2 rounded-full capitalize">
                                {cat.replace("_", " ")}: {cnt}
                              </span>
                            ))}
                          </div>
              ...
              <div className="border border-white/5 bg-zinc-950/40 rounded-2xl p-5 flex flex-col gap-4">
                ...
                {analytics.officers.length === 0 ? (
                  <p className="text-[10px] text-zinc-500 text-center py-6 font-bold">No officers available.</p>
                ) : (
                  <div className="space-y-3 overflow-y-auto max-h-[400px] pr-1">
                    {analytics.officers.map((off) => (
                      <div key={off.uid} className="flex justify-between items-center p-2.5 rounded-xl border border-white/5 bg-zinc-900/20 text-xs hover:border-red-500/10 transition-all">
                        <div className="flex flex-col gap-0.5">
                          <span className="font-extrabold text-zinc-200 line-clamp-1">{off.name}</span>
                          <span className="text-[9px] text-zinc-400 font-semibold">{off.department}</span>
                          <span className="text-[9px] text-zinc-500 flex items-center gap-1.5 font-medium mt-1">
                            Resolved: <strong className="font-mono text-zinc-300 font-extrabold">{off.resolvedCount}</strong>
                            {off.averageResolutionTime > 0 && (
                              <>
                                | Avg SLA: <strong className="font-mono text-zinc-300 font-extrabold">{off.averageResolutionTime}h</strong>
                              </>
                            )}
                          </span>
                        </div>

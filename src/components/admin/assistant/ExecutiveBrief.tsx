...
export const ExecutiveBrief: React.FC<ExecutiveBriefProps> = ({ forceRefresh, onBriefGenerated }) => {
  ...
  return (
    <div className="flex flex-col gap-6 h-full">
      {/* Header card */}
      <div className="border border-white/10 rounded-2xl bg-black/40 p-5 backdrop-blur-md flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          disabled={loading}
          className="px-4 py-2 text-xs font-bold text-white bg-gradient-to-r from-red-500 to-rose-500 hover:from-red-600 hover:to-rose-600 rounded-xl transition-all flex items-center gap-2 self-start sm:self-auto cursor-pointer shadow-lg disabled:opacity-50"
        >
          {loading ? (
            <>
      ...
      {loading && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-pulse">
          {[...Array(6)].map((_, idx) => (
            <div key={idx} className="border border-white/5 bg-zinc-950/20 rounded-xl p-4 h-36 flex flex-col gap-3">
              ...
              <div className="h-2 w-[90%] rounded bg-zinc-900" />
              <div className="h-2 w-[75%] rounded bg-zinc-900" />
            </div>
          ))}
        </div>
      )}
      ...
      {!loading && brief && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {sections.map((sec, idx) => (
            <div
              key={idx}
              className={`border ${sec.borderColor} ${sec.bgColor} rounded-2xl p-4.5 backdrop-blur-sm flex flex-col gap-2 transition-all duration-300 hover:border-white/10`}
            >
              <div className="flex items-center gap-2">
                <div className="p-1 rounded bg-zinc-950 border border-white/5">
                  {sec.icon}
                </div>
      ...
      {!loading && !brief && !error && (
        <div className="flex-1 flex flex-col items-center justify-center py-20 border border-dashed border-white/5 rounded-2xl bg-zinc-950/10">

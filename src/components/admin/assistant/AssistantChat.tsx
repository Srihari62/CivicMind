...
export const AssistantChat: React.FC<AssistantChatProps> = ({ forceRefresh }) => {
  ...
  return (
    <div className="flex flex-col h-[650px] border border-white/10 rounded-2xl bg-black/40 backdrop-blur-md overflow-hidden shadow-2xl">
      {/* Chat header */}
      <div className="border-b border-white/5 bg-zinc-950/40 px-5 py-4 flex items-center justify-between">
        ...
        {messages.length > 0 && (
          <button
            ...
            className="text-zinc-500 hover:text-red-400 p-1.5 rounded-lg hover:bg-white/5 transition-all cursor-pointer"
          ...
          <>
            ...
            {loading && (
              <div className="flex w-full gap-3 justify-start">
                ...
                <div className="border border-white/5 bg-zinc-900/60 text-zinc-400 rounded-2xl px-4 py-3 text-xs backdrop-blur-sm flex items-center gap-2 font-medium">
      ...
      {/* Input form */}
      <form onSubmit={handleFormSubmit} className="border-t border-white/5 p-4 bg-zinc-950/20 flex gap-3 items-center">
        <input
          type="text"
          value={input}
          onChange={(e) => setInput(e.target.value)}
          placeholder="Ask about incidents, workloads, duplicate trends..."
          className="flex-1 bg-zinc-900 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-red-500 transition-all font-medium"
          disabled={loading}
        />
        <button
          type="submit"
          disabled={!input.trim() || loading}
          className="p-2.5 rounded-xl bg-gradient-to-r from-red-500 to-rose-500 hover:from-red-600 hover:to-rose-600 text-white transition-all disabled:opacity-50 flex items-center justify-center cursor-pointer shadow-md"
        >
